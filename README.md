...
QoS (Quality of Service) 配置参数：

- `image_qos_reliability`：图像订阅可靠性策略
  - `best_effort`：尽力而为，不保证送达，低延迟（推荐，适合高频传感器数据）
  - `reliable`：保证送达，可能因重传导致 CPU 高负载时阻塞
- `image_qos_history_depth`：图像订阅历史队列深度（KEEP_LAST 模式）
  - `1`：仅保留最新一帧，最低延迟（推荐）
  - `2-10`：适度缓冲，应对短时处理延迟
- `image_qos_durability`：图像订阅持久性
  - `volatile`：只对订阅后的消息负责（推荐）
  - `transient_local`：保留最近的消息给新订阅者

**详细 QoS 配置说明请参考**: [QOS_CONFIGURATION.md](QOS_CONFIGURATION.md)
