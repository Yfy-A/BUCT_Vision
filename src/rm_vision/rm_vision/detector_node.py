...
class DetectorNode(Node):
    ...
    def __init__(self):
        ...
        # QoS 配置参数
        self.declare_parameter('image_qos_reliability', 'best_effort')  # 'reliable' | 'best_effort'
        self.declare_parameter('image_qos_history_depth', 1)  # 图像订阅队列深度
        self.declare_parameter('image_qos_durability', 'volatile')  # 'volatile' | 'transient_local'
        ...
        image_qos = QoSProfile(
            reliability=ReliabilityPolicy.BEST_EFFORT if self.image_qos_reliability == 'best_effort'
                        else ReliabilityPolicy.RELIABLE,
            history=HistoryPolicy.KEEP_LAST,
            depth=self.image_qos_history_depth,
            durability=DurabilityPolicy.VOLATILE if self.image_qos_durability == 'volatile'
        ...
        self.get_logger().info(
            f"Image subscription QoS: reliability={self.image_qos_reliability}, "
            f"history_depth={self.image_qos_history_depth}, durability={self.image_qos_durability}"
        )
    ...
    def _load_parameters(self):
        ...
        # 读取 QoS 配置参数
        self.image_qos_reliability = str(self.get_parameter('image_qos_reliability').value).lower()
        self.image_qos_history_depth = int(self.get_parameter('image_qos_history_depth').value)
        self.image_qos_durability = str(self.get_parameter('image_qos_durability').value).lower()
